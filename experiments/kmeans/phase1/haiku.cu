/*
 * K-Means CUDA implementation (Phase 2 optimized)
 *
 * Key optimizations over phase 1:
 *   1. Transposed feature layout [nfeatures][npoints] for coalesced global reads
 *   2. Centroids in __constant__ memory (680 bytes, broadcast cache)
 *   3. Register caching: each thread loads its 34 features ONCE into registers,
 *      then iterates over all 5 clusters using register values (no re-read)
 *   4. __ldg() read-only texture cache hint for feature loads
 *   5. Full #pragma unroll on the 34-element inner feature loop
 *
 ...
 * All threads in a warp read the same centroid element => broadcast, zero extra bandwidth.
 */
#define MAX_CLUSTERS 5
#define MAX_FEATURES 34
/* Compile-time feature count for full unroll */
#define NFEATURES    34
 ...
 *
 * Features are stored TRANSPOSED as [nfeatures][npoints] so that threads in a
 * warp (consecutive point indices i, i+1, ..., i+31) load consecutive floats
 * from each feature row -- fully coalesced 128-byte cache-line transactions.
 *
 * Each thread:
 *   1. Loads all NFEATURES feature values into registers in one coalesced pass.
 *   2. Iterates over nclusters, computing squared distance entirely from registers
 *      and constant memory -- no repeat global loads.
 */
__global__ void kmeans_assign(
    ...
    /* Load all features into registers (NFEATURES=34 floats, coalesced global reads) */
    ...
    /* Distance to each centroid, computed entirely from registers + constant memory */
    for (int c = 0; c < nclusters; c++) {
        float dist = 0.0f;
        const float *cptr = c_clusters + c * nfeatures;
        #pragma unroll
        for (int f = 0; f < NFEATURES; f++) {
            float d = fval[f] - cptr[f];
            dist += d * d;
        }
        if (dist < min_dist) {
            min_dist = dist;
...
int main(int argc, char **argv) {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        /* GPU: assign step */
        kmeans_assign<<<blocks, threads_per_block>>>(
