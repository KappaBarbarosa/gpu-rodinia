/*
 * lavaMD — Phase 1 CUDA port (global memory only, straightforward data-parallel).
 *
 * One thread-block per home box, one thread per home particle. Each thread loops
 * over the home box + neighbour boxes, reading rv/qv from global memory, and
 * accumulates its own fv 4-vector. No shared memory / tiling (phase 2).
 *
