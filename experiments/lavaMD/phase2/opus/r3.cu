/*
 * lavaMD — Phase 2 CUDA port, round 3 (CONVERGED design, preserved from r2).
 *
 * One thread-block per home box, one thread per home particle (block == 100).
 * The 100 threads cooperatively stage each interaction box (home + up to 26
 * neighbours) into a packed __shared__ float4 sRV[100] (x,y,z,v) plus scalar
 * sQ[100], then the inner j-loop reads only shared memory: one 128-bit
 * ld.shared.v4 per partner. __expf replaces expf (verified within tolerance).
 * The fixed-bound (100) inner loop is fully unrolled.
 *
 * Round-3 status: optimization space is exhausted. The kernel is genuinely
 * __expf/SFU-throughput bound — one irreducible exp per partner that cannot
 * be removed without breaking the golden match. __launch_bounds__(100, 8)
 * raises the ptxas register budget 40->55 (0 spills, verified -Xptxas -v),
 * giving the unrolled inner loop more registers to overlap __expf latency:
 * measured ~0.710 -> ~0.682 ms (~4%) with --fmad=false output diffed against
 * golden_10.out (max_abs_err 4.89e-4, max_rel_err 9.44e-5, 0 mismatches).
 * Rejected variants on this HW: minBlocks sweep 4..16 (plateau 0.678-0.685),
 * explicit 2-way ILP split (0.703, worse + reorders acc), q packed in
 * float4.w (0.691, worse). Remaining gains < 5% threshold. This file keeps
 * the verified-best kernel unchanged.
 *
 ...
 *
 * __launch_bounds__(100, 8): minBlocks=8 lets ptxas raise the per-thread
 * register budget from 40 to 55 (no spills; sm_86 allows up to 64 reg/thread
 * at 800 resident threads), which gives the fully-unrolled SFU-bound inner
 * loop more registers to overlap __expf latency. Measured ~0.710 -> ~0.682 ms
 * (~4%) with bit-identical --fmad=false output (verify mismatches=0). */
__global__ void __launch_bounds__(NUMBER_PAR_PER_BOX, 8)
                lavaMD_kernel(long number_boxes,
                              fp a2,
                              ...
                              FOUR_VECTOR * __restrict__ fv)
{
    ...
    /* Stage the partner record (x,y,z,v) as a packed float4 so the hot inner
     * loop issues a single 128-bit ld.shared.v4 per partner; q is a scalar
     * touched once per partner so it stays in a separate shared array. */
    __shared__ float4 sRV[NUMBER_PAR_PER_BOX];   /* (x,y,z,v) packed, 16B vec load */
    ...
    for (int k = 0; k < (1 + nn); k++) {
        ...
        __syncthreads();
        FOUR_VECTOR rBi = rv[first_j + i];
        sRV[i] = make_float4(rBi.x, rBi.y, rBi.z, rBi.v);
        sQ[i]  = qv[first_j + i];
        __syncthreads();

        #pragma unroll
        for (int j = 0; j < NUMBER_PAR_PER_BOX; j++) {
            float4 pj = sRV[j];                 /* single 128-bit shared load */
            fp r2  = rA.v + pj.w - (rA.x*pj.x + rA.y*pj.y + rA.z*pj.z);
            fp vij = __expf(-(a2 * r2));
            fp qj  = sQ[j];
            fp qv_ = qj * vij;            /* qj * vij, used for acc_v */
            fp qfs = qv_ + qv_;           /* qj * 2 * vij = force prefactor */
            acc_v += qv_;
            acc_x += qfs * (rA.x - pj.x);
            acc_y += qfs * (rA.y - pj.y);
            acc_z += qfs * (rA.z - pj.z);
