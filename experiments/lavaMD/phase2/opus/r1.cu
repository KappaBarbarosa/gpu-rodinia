/*
 * lavaMD — Phase 2 CUDA port, round 1 (shared-memory tiled, converged design).
 *
 ...
 * neighbours) into __shared__ sB[100]/sQ[100] (100x reuse per box), then the
 * inner j-loop reads only shared memory. Neighbour offset is computed directly
 * (number*100) instead of a second global load. __expf replaces expf (verified
 * within tolerance). The fixed-bound inner loop is fully unrolled.
 *
 * Round-2 micro-tuning over p2r1 (nsys kernel 0.796ms -> 0.72ms, ~9% faster,
 * verified 0 mismatches, max_rel 9.4e-5, --fmad=false):
 *  - Stage the partner record as a packed __shared__ float4 (x,y,z,v): the hot
 *    inner loop now issues ONE 128-bit ld.shared.v4 per partner instead of
 *    3x32-bit + 1x32-bit scalar shared loads. This also dropped the register
 *    footprint 44 -> 40, so it costs no occupancy.
 *  - Strength-reduce the force prefactor: compute qv = qj*vij once (reused for
 *    acc_v), then qfs = qv+qv (== qj*2*vij) and three FMAs (qfs*(rA-pj)) for
 *    x/y/z. This removes two multiplies per inner iteration versus the previous
 *    qj*(2*vij*d) form. Reassociation stays inside the verify tolerance.
 *
 * The inner loop is now SFU-bound on the single __expf per partner; reducing
 * registers (maxrregcount 32) raised occupancy but did NOT improve time, and
 * occupancy is already ~78% (40 regs / 100 thr/block), confirming the exp
 * throughput is the wall. Structural levels (occupancy past 100 thr/block,
 * cross-box tile sharing) remain blocked by the one-block-per-box /
 * one-thread-per-particle mapping that pins correctness to the serial golden.
 *
