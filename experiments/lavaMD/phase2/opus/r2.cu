/*
 * lavaMD — Phase 2 CUDA port, round 2 (shared-memory tiled, CONVERGED design).
 *
 ...
 *
 * Round-2 status: prior measured feedback established this kernel is
 * SFU/exp-throughput bound on the single __expf per partner (1 mul + 1 add +
 * 3 FMA + 1 vec-load around it). Forcing maxrregcount=32 raised occupancy
 * ~78%->100% with NO time gain, confirming the wall is exp throughput, not
 * occupancy or memory. Reducing precision (use_fast_math beyond __expf, fewer
 * exp evals) would break the serial-golden match; the one-block-per-box /
 * one-thread-per-particle mapping that pins correctness blocks cross-box tile
 * reuse and wider blocks. This file preserves the verified-best converged
 * kernel: packed float4 partner staging + strength-reduced force prefactor
 * (qv_=qj*vij reused for acc_v, qfs=qv_+qv_, three FMAs qfs*(rA-pj)).
 *
