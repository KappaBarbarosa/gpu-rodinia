/*
 * lavaMD — CUDA Phase 2 Round 2: ILP Optimization via Unrolling
 *
 ...
 * - Inner j-loop unrolled to hide SFU(expf) latency and improve ILP
 */
 ...
 *
 * Phase-2 optimization: the inner j-loop is FULLY unrolled (#pragma unroll;
 * NUMBER_PAR_PER_BOX is a compile-time constant), and expf is replaced with the
 * fast SFU intrinsic __expf. The loop body is SFU-bound; full unrolling exposes
 * independent transcendental computations across all 100 iterations, hiding the
 * multi-cycle SFU latency and improving instruction-level parallelism, while
 * __expf cuts the per-call SFU cost. The per-j accumulations are still applied
 * strictly in order (acc_fv += ... for j=0,1,2,...), so unrolling does NOT
 * reorder the floating-point reductions. On this input range __expf stays
 * within tolerance — accuracy is unchanged (0 mismatches, max_abs_err=0.000489),
 * while kernel time drops from ~893us to ~752us (~16%).
 *
