...
#define PYRAMID_HEIGHT 56   /* rows fused per launch; tuned (owned=272 cols/block).
                               Swept BS in {128..1024}, PH in {16..200} by median-of-3
                               nsys kernel time at cols=100000,rows=1000 on sm_86:
                               384/56 -> 1.188ms is the minimum. The landscape is
                               DRAM-bandwidth bound (wall = rows*cols*4 read with
                               BLOCK_SIZE/owned redundancy) with sharp tail-wave
                               cliffs (e.g. PH=58 jumps to 1.53ms), so this pocket
                               is a stable local optimum, not a smooth gradient. */
