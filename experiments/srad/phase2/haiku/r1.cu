/*
 * SRAD (Speckle Reducing Anisotropic Diffusion) — CUDA Phase 2 port, Round 1.
 *
...
/* FUSED prepare+update with thread-coarsening. Each thread computes 2 output
 * rows, amortizing the c halo recompute. BY=4 with 2-row coarsening maps to
 * effective BY=8 occupancy with better ILP. J tile is now (2*BY+3) x (BX+3) =
 * 11 x 35 (for BY=4). c tile is (2*BY+1) x (BX+1) = 9 x 33.
 */
