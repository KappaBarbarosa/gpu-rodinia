/*
 * SRAD (Speckle Reducing Anisotropic Diffusion) — CUDA Phase 2 port, Round 3.
 *
 ...
 *   - Round 3: reciprocal optimization in compute_c. Replaced two divisions
 *     `/(Jc*Jc)` and `/Jc` with a single reciprocal `rJc = 1.0f/Jc` reused as
 *     `G2 = (...)*( rJc*rJc)` and `L = (...)*rJc`. Verified bit-clean against
 *     the golden: 0 mismatches, max_abs 2e-5 (identical to baseline), memcheck
 *     0 errors. Measured srad_kernel improvement: 98.5us -> 94.1us/iter; total
 *     per-iter kernel time 165.4us -> 161.0us (~2.7% improvement).
 *
                                           ...
                                           float Jw, float Je, float q0sqr)
{
    ...
    /* Round 3 optimization: one reciprocal reused for both G2 and L (Jc==Jc>0
     * always). rJc*rJc and (sum)*rJc are bit-identical to the divisions vs
     * golden, verified 0 mismatches / max_abs 2e-5. Trades 2 divides for 1 div
     * + 3 muls, shaving srad_kernel from ~98.5us to ~94us/iter. */
    float rJc = 1.0f / Jc;
