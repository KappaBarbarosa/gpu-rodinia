/*
 * SRAD (srad_v2) — Phase 2 r3: two-kernel-per-iteration scheme.
 *
 * HISTORY / WHY THIS SHAPE:
 *  - r2 ran two kernels per iteration (reduce_roi<<<1,1>>> + srad_fused) and was
 *    CORRECT (matched the serial golden bit-closely under --fmad=false).
 *  - An attempted r3 fused all iterations into ONE persistent cooperative-groups
 *    kernel using grid.sync(). That FAILED at runtime: a full image of
 *    rows*cols/(16*16) tiles needs gridBlocks=16384 co-resident blocks, but the
 *    device can only co-reside numBlocksPerSM*numSM = 138 blocks. Cooperative
 *    launch requires the WHOLE grid resident, so the launch was impossible and
 *    the program produced no output (verify_ok=false).
 *
 * This r3 reverts to the proven correct two-kernel-per-iteration scheme. There
 * is NO grid-wide barrier dependency, so it runs on any image size. The serial
 * ROI float-add order is preserved exactly.
 *
 * Per-pixel math is computed in double then stored to float to match the serial
 * golden bit-closely under --fmad=false. The ROI reduction is a single
 * continuous left-to-right row-major float running sum performed by one thread:
 * float add is non-associative and q0sqr feeds every pixel, so the order MUST
 * NOT change. We do NOT parallelize this sum.
 */
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
// ROI reduction: ONE thread performs the exact continuous left-to-right
// row-major float running sum, then computes q0sqr. Bit-identical to r2.
...
// One shared-memory tiled fused prepare+update step over the whole image.
// Reads Jin (OLD), writes Jout (NEW). Identical numerics to r2's srad_fused.
__global__ void srad_fused(const float *__restrict__ Jin,
...
int main(int argc, char **argv)
{
    ...
    // Host-side ping-pong: even iter reads d_J writes d_J2, odd swaps.
    for (int iter = 0; iter < niter; iter++) {
        const float *Jin  = (iter & 1) ? d_J2 : d_J;
        float       *Jout = (iter & 1) ? d_J  : d_J2;

        reduce_roi<<<1, 1>>>(Jin, cols, r1, r2, c1, c2, size_R, d_q0);
        srad_fused<<<grid, block>>>(Jin, Jout, rows, cols, lambda, d_q0);
    }
    CK(cudaGetLastError());
    ...
    // Final buffer: even niter -> d_J, odd niter -> d_J2.
