...
// ROI reduction: cooperatively loads ROI into shared memory, then thread 0
// runs the exact serial-order add chain to match the golden reference.
...
// PREPARE kernel: uses float2 packing for diff arrays.
// dNS[k] = {dN, dS}, dWE[k] = {dW, dE}
// This halves the number of diff-array stores vs. 4 separate arrays.
__global__ void kernel_prepare(
    const float * __restrict__ J,
    float2 * __restrict__ dNS,
    float2 * __restrict__ dWE,
    ...
    const float * __restrict__ g_q0sqr)
{
    ...
    // Pack diffs into float2 for fewer memory transactions
    dNS[k] = make_float2(dn, ds);
    dWE[k] = make_float2(dw, de);
...
// UPDATE kernel: reads packed float2 diff arrays + tiled c[].
__global__ void kernel_update(
    float * __restrict__ J,
    const float2 * __restrict__ dNS,
    const float2 * __restrict__ dWE,
    ...
    float lambda)
{
    ...
    float2 ns = __ldg(&dNS[k]);
    float2 we = __ldg(&dWE[k]);

    float D = cN*ns.x + cS*ns.y + cW*we.x + cE*we.y;
...
int main(int argc, char **argv)
{
    ...
    float  *d_J, *d_c, *d_q0sqr;
    float2 *d_dNS, *d_dWE;

    cudaMalloc(&d_J,    size_I * sizeof(float));
    cudaMalloc(&d_dNS,  size_I * sizeof(float2));
    cudaMalloc(&d_dWE,  size_I * sizeof(float2));
    ...
    cudaFree(d_J); cudaFree(d_dNS); cudaFree(d_dWE);
